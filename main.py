...
@click.option(
    "--chunk-size", default=2, help="Number of PDFs to process in each worker"
)
...
def parse_pdfs(parallel, chunk_size, max_workers):
    ...
    click.echo(
        f"Parsing PDFs (parallel={parallel}, chunk_size={chunk_size}, max_workers={max_workers})"
    )
    pipeline.parse_pdf_reports(
        parallel=parallel, chunk_size=chunk_size, max_workers=max_workers
    )


@cli.command()
@click.option(
    "--max-workers", default=10, help="Number of workers for table serialization"
)
